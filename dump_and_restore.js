...
// Tables in dependency order (parents first so FK constraints don't fail)
const ALL_TABLES = [
  'users',
  'items',
...
// Generated columns that must be excluded from INSERT
...
async function main() {
  ...
  console.log('║   Old Project: cifzlkspxjghpgxhrwkg                      ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
  ...
  sql += `-- Source: cifzlkspxjghpgxhrwkg\n`;
  sql += `-- Target: wkyakaunbejmuzqnvgno\n`;
  sql += `-- ============================================================================\n\n`;
  ...
  const outPath = 'c:\\Users\\LuckyGold\\Desktop\\AKV - Copy\\COMPLETE_DATABASE_DUMP.sql';
  ...
  for (const [table, { rows }] of Object.entries(results)) {
    console.log(`  ${table.padEnd(25)} ${String(rows.length).padStart(6)} rows`);
  }
  ...
  console.log('\n✅ Dump complete!');
  console.log('\n📋 NEXT STEPS:');
  console.log('   1. Open https://wkyakaunbejmuzqnvgno.supabase.co');
  console.log('   2. Go to SQL Editor');
  console.log('   3. Copy the contents of COMPLETE_DATABASE_DUMP.sql');

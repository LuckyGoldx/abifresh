...
export default function ExpensesPage() {
  ...
  const fetchExpenses = async () => {
    try { const r = await api.get('/api/staff/expenses'); setExpenses(r.data); } catch { addToast('Failed to fetch expenses', 'error'); } finally { setIsLoading(false); }
  };
  ...
  const submitExpenseRequest = async () => {
    setSubmitting(true);
    try {
      await api.post('/api/staff/expenses/create', { amount: parseFloat(amount), category, description, expense_date: expenseDate });

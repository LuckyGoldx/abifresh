...
export default function SalesPaymentsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent | React.MouseEvent) => {
    ...
    // Compute amount fresh from selected items
    ...
    try {
      ...
      const response = await fetch('/api/sales/payments/request', {
        method: 'POST',
            ...
            <div>
              ...
              <div className="border dark:border-gray-700 rounded-lg max-h-60 overflow-x-auto overflow-y-auto">
                {soldItems.length > 0 ? (
                            ...
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                onBlur={(e) => {
                                  ...
                                  const finalVal = Math.min(rounded, item.quantity);
                                  setSelectedQuantities(prev => ({ ...prev, [item.id]: finalVal }));
                                  if (finalVal > 0) {
                                    setSelectedItems(prev => prev.includes(item.id) ? prev : [...prev, item.id]);
                                  } else {
                                    setSelectedItems(prev => prev.filter(id => id !== item.id));
                  ...
                  <p className="text-center py-4 text-gray-500">No items sold yet</p>
                )}
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-gray-900 rounded-lg max-w-2xl w-full max-h-96 overflow-y-auto p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">Your Name</p>
            ...
            <div className="bg-blue-50 dark:bg-blue-900 p-3 rounded text-sm text-blue-800 dark:text-blue-200">
              <strong>Note:</strong> This payment will be sent to admin for approval. It will remain as "Pending" until admin approves it. Once approved, it will move to "Approved Payments" and you'll receive a notification.
            </div>

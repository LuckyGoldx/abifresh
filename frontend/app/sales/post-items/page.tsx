...
export default function PostItemsPage() {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const response = await api.get('/api/sales/post-items');
  ...
  const handlePostItems = async () => {
    ...
    const invalidItem = cart.find(item => !item.post_quantity || item.post_quantity < 0.5 || item.post_quantity % 0.5 !== 0);
    if (invalidItem || Object.keys(quantityInputs).length > 0) {
      setToast({ message: 'Please enter valid quantities (multiples of 0.5)', type: 'error' });
      return;
    }
  ...
  const confirmPostItems = async () => {
    ...
    try {
      ...
      console.log('📤 [Sales] Posting items with data:', postData);
  ...
  const filteredHistory = history.filter(item => {
    ...
    return matchesStaff && matchesStatus && matchesItem && matchesDate;
  });
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {isHistoryLoading ? (
                    ...
                    <tr>
                      <td colSpan={6} className="px-6 py-12 text-center text-gray-400">No matching history found</td>
                    </tr>
                  ) : (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="bg-gradient-to-r from-pink-500 to-pink-600 px-6 py-6 text-center">
              <h2 className="text-2xl font-bold text-white mb-1">🚀 Review Your Posting</h2>
              <p className="text-pink-100 text-sm">Please double-check everything before we transfer these items to staff.</p>
            </div>

            <div className="p-6 space-y-6">
              {/* Staff Info & Location */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-pink-50 dark:bg-pink-900/20 rounded-xl p-4 border-2 border-pink-100 dark:border-pink-800 shadow-sm">
                  <p className="text-xs font-bold text-pink-600 dark:text-pink-400 uppercase tracking-wider mb-2">Recipient Staff</p>
                  <p className="text-xl font-black text-gray-900 dark:text-white">
                    {staffList.find(s => s.id === selectedStaff)?.full_name}
                  </p>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 font-medium">
                    Role: {displayRoleName(staffList.find(s => s.id === selectedStaff)?.role || '')}
                  </p>
                </div>
                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border-2 border-blue-100 dark:border-blue-800 shadow-sm">
                  <p className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider mb-2">Designated Location</p>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 font-medium">
                    Inventory will be tracked at this location
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                <button
                  ...
                  disabled={isPosting}
                  className="flex-1 px-6 py-4 border-2 border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 font-bold transition disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  No, Go Back
                </button>
                <button
                  ...
                  disabled={isPosting}
                  className="flex-[2] px-6 py-4 bg-pink-600 hover:bg-pink-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl font-black shadow-lg shadow-pink-500/30 transition flex items-center justify-center gap-2"
                >
                  {isPosting ? (
                    <>

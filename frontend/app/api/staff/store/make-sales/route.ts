...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (const item of items) {
      ...
      const itemLocation = sold_outside_jalingo ? 'Outside Jalingo' : 'Inside Jalingo';
      ...
      // Get staff store entry for the specific location
      ...
      if (quantityAvailable < quantity) {
        return NextResponse.json(
          { error: `Insufficient stock in ${itemLocation}. Available: ${quantityAvailable}, Requested: ${quantity}` },
          { status: 400 }
        );
      }
